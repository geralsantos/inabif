  ...
  public function mostrar_matrices(){
    ...
    public function descargar_reporte_matriz_general_2(){
      ...
      if (!$matriz_consolidado) {
        ...
        $modulos = "select cu.*,cda.* from CarIdentificacionUsuario cu 
        left join CarDatosAdmision cda on(cu.residente_id=cda.residente_id)
        left join CarCondicionIngreso cci on(cu.residente_id=cci.residente_id) 
        left join residente re on re.id = 97";
        $modulos = $modelo->executeQuery($modulos);
        $residentes = array();
        $grupo_html = "";
