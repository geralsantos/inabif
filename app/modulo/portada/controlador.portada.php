...
class portada extends App{
  ...
  public function buildTreeHtml($elements,$opt="")
  {
	...
	public function ejecutar_consulta_lista(){
	  ...
	  echo $sql = "SELECT max(re.id) as id,max(re.nombre) as nombre,max(re.apellido_p) as apellido_p,max(re.apellido_m) as apellido_m,max(".$campo.") as dni_residente FROM Residente re ".$left_join." WHERE  re.ESTADO=1 ".$where." ".$where_join." group by re.id ORDER BY re.Id desc";
	  $res = $modelo->executeQuery( $sql );
	  if ($res) {
	echo json_encode(array( "data"=>$res )) ;
	  }else{
 ...
 //ob_start();
  ...
  public function consulta_reniec(){
    ...
    try {
      ...
      $path = 'https://ws5.pide.gob.pe/Rest/Reniec/Consultar?nuDniConsulta=76934495&nuDniUsuario='.NUDNIUSUARIO.'&nuRucUsuario='.NURUCUSUARIO.'&password='.PASSWORD;
