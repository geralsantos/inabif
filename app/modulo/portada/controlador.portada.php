	...
	public function ejecutar_consulta_lista(){
      ...
      $tipo_centro_id = $_SESSION["usuario"][0]["TIPO_CENTRO_ID"];
        if ($tipo_centro_id == PPD) {
		  ...
		  $sql = "SELECT DISTINCT " .$campo.",re.* as dni_residente FROM Residente re ".$left_join." WHERE  re.ESTADO=1  AND re.centro_id = ".$_SESSION["usuario"][0]["ID_CENTRO"]." ORDER BY re.Id desc";
		  $res = $modelo->executeQuery( $sql );
		  if ($res) {
			echo json_encode(array( "data"=>$res )) ;
		  }else{
