  ...
  public function mostrar_matrices(){
    ...
    public function descargar_reporte_matriz_general_2(){
      ...
      if (!$matriz_consolidado) {
        ...
        $fecha = " BETWEEN UPPER('".date("y-M-01",strtotime($periodo_anio."-".$periodo_mes))."') AND UPPER('".date(("y-M-".$last_day),strtotime($periodo_anio."-".$periodo_mes))."')";
...
/*ccs.Nom_Entidad, ccs.Cod_Linea ,ccs.Linea_Intervencion , ccs.Cod_Servicio , ccs.Nom_Servicio, ccs.Ubigeo_Ine, ccs.Departamento_CAtencion, ccs.Provincia_CAtencion, ccs.Distrito_CAtencion, ccs.Centro_Poblado, ccs.Centro_Residencia*/
        ...
        ,centro_atencion ca 
        ,residente re
        where cu.residente_id(+)=re.id and ca.id(+)= re.centro_id and cda.residente_id(+)=re.id and cci.residente_id(+)=re.id and csn.residente_id(+)=re.id and csm.residente_id(+)=re.id and ct.residente_id(+)=re.id and cac.residente_id(+)=re.id and cap.residente_id(+)=re.id and cec.residente_id(+)=re.id and cts.residente_id(+)=re.id and cas.residente_id(+)=re.id and cep.residente_id(+)=re.id and cee.residente_id(+)=re.id and ces.residente_id(+)=re.id and ctf.residente_id(+)=re.id and cen.residente_id(+)=re.id and cets.residente_id(+)=re.id and ceg.residente_id(+)=re.id and re.tipo_centro_id=1 and cu.fecha_creacion ('.$fecha.') and ca.fecha_creacion ('.$fecha.') and cda.fecha_creacion ('.$fecha.') and cci.fecha_creacion ('.$fecha.') and csn.fecha_creacion ('.$fecha.') and csm.fecha_creacion ('.$fecha.') and ct.fecha_creacion ('.$fecha.') and cac.fecha_creacion ('.$fecha.') and cap.fecha_creacion ('.$fecha.') and cec.fecha_creacion ('.$fecha.') and cts.fecha_creacion ('.$fecha.') and cas.fecha_creacion ('.$fecha.') and cep.fecha_creacion ('.$fecha.') and cee.fecha_creacion ('.$fecha.') and ces.fecha_creacion ('.$fecha.') and ctf.fecha_creacion ('.$fecha.') and cen.fecha_creacion ('.$fecha.') and cets.fecha_creacion ('.$fecha.') and ceg.fecha_creacion ('.$fecha.')';
        $modulos = $modelo->executeQuery($modulos);
        $residentes = array();
        $grupo_html = "";
