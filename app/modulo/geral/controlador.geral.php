...
class geral extends App{
  ...
  $centro = $modelo->executeQuery($query)[0];
    switch ($tipo_centro_id) {
		...
		$campos = "'".$centro["CODIGO_ENTIDAD"]."' as COD_ENTIDAD,'".$centro["NOMBRE_ENTIDAD"]."' as NOM_ENTIDAD,'".$centro["CODIGO_LINEA"]."' as COD_LINEA,'".$centro["LINEA_INTERVENCION"]."' as LINEA_INTERVENCION,'".$centro["COD_SERV"]."' as COD_SERVICIO,'".$centro["COD_SERV"]."' as NOM_SERVICIO,'".$centro["UBIGEO"]."' as UBIGEO_CA,(select NOMDEPT from ubigeo where CODDIST='".$centro["UBIGEO"]."') as DEPAR_CA,(select NOMPROV from ubigeo where CODDIST='".$centro["UBIGEO"]."') as PROVIN_CAR,(select NOMDIST from ubigeo where CODDIST='".$centro["UBIGEO"]."') as DISTR_CA,(select NOMDIST from ubigeo where CODDIST='".$centro["UBIGEO"]."') as CCPP_CA,'".$centro["AREA_RESIDENCIA"]."' as AREA_RES_CA,'".$centro["COD_CA"]."' as COD_CA,'".$centro["NOM_CA"]."' AS NOM_CA,max(re.id) as COD_RESIDENTE,max(re.NOMBRE) AS NOM_RESIDENTE, max(re.apellido_p) AS APE_PAT_RESIDENTE, max(re.apellido_m) as APE_MAT_RESIDENTE, max(tdi.nombre)  as TIP_DOC_USU, max(ci.Numero_Documento) as NRO_DOC_USU,max(iu.Fecha_Nacimiento) as FEC_NAC_RESIDENTE, max(iu.Edad) as EDAD_RESIDENTE,(CASE iu.Sexo WHEN 'h' THEN 'Hombre' ELSE 'Mujer' END) as SEXO_RESIDENTE, '' as DIR_USU, max(re.distrito_naci_cod) as UBIGEO_USU,(select NOMDEPT from ubigeo where CODDIST=re.distrito_naci_cod) as DEP_USU,(select NOMPROV from ubigeo where CODDIST=re.distrito_naci_cod) as PROV_USU,(select NOMDIST from ubigeo where CODDIST=re.distrito_naci_cod) as DIST_USU,max(ci.FECHA_CREACION) as FEC_INGRESO, '' as PERFIL_ING,max(da.Institucion_derivado) as VIA_ING, max(sn.Discapacidad) as FLG_DISCAP, max(re.Estado) as EST_RESIDENTE,max(eg.Fecha_Egreso) as FEC_EGRE, max(eg.Motivo_Egreso) as MOTIVO_EGRE, max(da.Fecha_Reingreso) as FEC_REING, '' as Grupo_etario,max(eg.Traslado) as TRASLADO, max(eg.Fallecimiento) as FALLECIMIENTO,max(eg.Reinsercion) AS Reinsercion, max(eg.Aus) as ASEGURAMIENTO_UNIVERSAL, max(eg.Constancia_Naci) as PARTIDA_DE_NACIMIENTO, max(eg.DNI) AS DNI,'' as EDUCACION ,max(eg.Reinsercion) AS Reinsercion_Familiar ";
		$from = " residente re inner join CarCondicionIngreso ci on (ci.Residente_Id=re.id) inner join CarIdentificacionUsuario iu on (iu.Residente_Id=re.id) inner join CarDatosAdmision da on (da.Residente_Id=re.id) inner join CarSaludNutricion sn on (sn.Residente_Id=re.id) inner join CarEgresoGeneral eg on (eg.Residente_Id=re.id) inner join pam_tipo_documento_identidad tdi on (tdi.id=ci.Tipo_Documento) order by re.id desc";
		break;
		case '2': /*pam*/
    $parent_id="27,43";
