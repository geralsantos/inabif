...
class geral extends App{
  ...
  $centro = $modelo->executeQuery($query)[0];
    switch ($tipo_centro_id) {
		...
		$from = " residente re inner join CarCondicionIngreso ci on (ci.Residente_Id=re.id) inner join CarIdentificacionUsuario iu on (iu.Residente_Id=re.id) inner join CarDatosAdmision da on (da.Residente_Id=re.id) inner join CarSaludNutricion sn on (sn.Residente_Id=re.id) inner join CarEgresoGeneral eg on (eg.Residente_Id=re.id) left join pam_tipo_documento_identidad tdi on (tdi.id=ci.Tipo_Documento)";
		break;
		case '2': /*pam*/
		$parent_id="27,43";
		break;
