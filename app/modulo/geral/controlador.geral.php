...
class geral extends App{
  ...
  public function campos_tipo_centro ($tipo_centro_id=""){
    ...
    switch ($tipo_centro_id) {
      case '1': /*ppd*/
      $campos = array('CarIdentificacionUsuario'=>'Ape_Paterno as "Apellido paterno",Ape_Materno as "Apellido materno", Nom_Usuario as "Nombre Usuario",(SELECT nombre FROM paises WHERE id=CarIdentificacionUsuario.Pais_Procencia) as "Pai­s de procedencia",(SELECT NOMDEPT FROM ubigeo WHERE coddist=CarIdentificacionUsuario.Distrito_Procedencia) as "Departamento de nac",(SELECT NOMPROV FROM ubigeo WHERE coddist=CarIdentificacionUsuario.Distrito_Procedencia) as "Provincia de nac del" ,(SELECT NOMPROV FROM ubigeo WHERE coddist=CarIdentificacionUsuario.Distrito_Procedencia) as "Distrito de nac",(CASE Sexo WHEN \'h\' THEN \'Hombre\' WHEN \'m\' THEN \'Mujer\' END) as Sexo,Fecha_Nacimiento,(SELECT nombre from pam_lengua_materna WHERE id = CarIdentificacionUsuario.Lengua_Materna) as "Lengua Materna"','CarDatosAdmision'=>'Fecha_Ingreso as "Fecha de Ingreso",(SELECT nombre FROM pam_instituciones WHERE id=CarDatosAdmision.Institucion_derivado) as "Entidad que lo deriva",Motivo_Ingreso  as "Motivo ingreso PRINCIPAL",Numero_Documento as "Número documento de Ingreso"','CarCondicionIngreso'=>'DNI as "DNI",(SELECT nombre FROM pam_tipo_documento_identidad WHERE id=CarCondicionIngreso.Tipo_Documento) as "Tipo de Documento", Numero_Documento as "Número de Documento", Posee_Pension as "Pensión", (SELECT nombre FROM pam_tipo_pension WHERE id=CarCondicionIngreso.Tipo_Pension) as "Tipo de Pensión",Lee_Escribe as "Sabe Leer y Escribir", (SELECT nombre FROM pam_nivel_educativo where id=CarCondicionIngreso.Nivel_Educativo) as "Nivel Educativo", (SELECT nombre FROM pam_clasif_socioeconomico where id = CarCondicionIngreso.Clasficacion_Socioeconomica) as "SISFOH",(SELECT nombre FROM pam_tipo_seguro_salud WHERE id=CarCondicionIngreso.Tipo_Seguro) as "Aseguramiento"','CarSaludNutricion'=>array('Discapacidad, Discapacidad_Fisica as "Presenta discap. física", Discapacidad_Intelectual as "Presenta discap. intelectual", Discapacidad_Sensorial as "Presenta discap. sensorial", Discapacidad_mental as "Presenta discap. mental",Carnet_CONADIS as "Carnet CONADIS",Motivo_Movilidad as "Dificultad Desplazamiento",Patologia1 as "Patología Crónica 1", (SELECT nombre FROM pam_tipo_patologia WHERE id =CarSaludNutricion.Tipo_Patologia1) as "Tipo de Patología",Especifique1  as "Especifique"','Nivel_Hemoglobina as "Nivel de Hemoglobina", Peso as "Peso (Kg.)",Talla as "Talla (m)",Estado_Nutricional as "Estado Nutricional(IMC)"'),'CarSaludMental'=>'');
      break;
      case '2': /*pam*/
      $campos = array('pam_datos_identificacion'=>'residente_apellido_paterno as "Apellido paterno",residente_apellido_materno as "Apellido materno", residente_nombre as "Nombre Usuario",(SELECT nombre FROM paises WHERE id=pam_datos_identificacion.pais_procedente_id) as "Pai­s de procedencia",(SELECT NOMDEPT FROM ubigeo WHERE coddist=pam_datos_identificacion.distrito_nacimiento_id) as "Departamento de nac",(SELECT NOMPROV FROM ubigeo WHERE coddist=pam_datos_identificacion.distrito_nacimiento_id) as "Provincia de nac del" ,(SELECT NOMPROV FROM ubigeo WHERE coddist=pam_datos_identificacion.distrito_nacimiento_id) as "Distrito de nac",(CASE sexo WHEN \'h\' THEN 2 WHEN \'m\' THEN 1 END) as Sexo,fecha_nacimiento,(SELECT nombre from pam_lengua_materna WHERE id = pam_datos_identificacion.lengua_materna) as "Lengua Materna"','pam_datos_admision_usuario'=>'fecha_ingreso_usuario as "Fecha de Ingreso",(SELECT nombre FROM pam_instituciones_deriva WHERE id=pam_datos_admision_usuario.institucion_deriva) as "Entidad que lo deriva",(SELECT nombre FROM pam_motivos_ingreso WHERE id = pam_datos_admision_usuario.motivo_ingreso_principal) as "Motivo ing. PRINCIPAL(exp)", (SELECT nombre FROM pam_motivos_ingreso WHERE id = pam_datos_admision_usuario.motivo_ingreso_secundario)as "Motivo ing. PRINCIPAL(real)" ,numero_documento_ingreo_car as "Número documento de Ingreso",perfil_ingreso as "Perfil de Ingreso"','pam_datosCondicionIngreso'=>'documento_entidad as "DNI",tipo_documento_entidad as "Tipo de Documento", numero_documento_ingreso as "Número de Documento", tipo_pension as "Tipo de pensión",leer_escribir as "Sabe Leer y Escribir", (SELECT nombre FROM pam_nivel_educativo where id=pam_datosCondicionIngreso.nivel_educativo) as "Nivel Educativo", (SELECT nombre FROM pam_clasif_socioeconomico where id = pam_datosCondicionIngreso.SISFOH) as "SISFOH",(SELECT nombre FROM pam_tipo_seguro_salud WHERE id=pam_datosCondicionIngreso.aseguramiento_salud) as "Aseguramiento"','pam_datos_saludnutric'=>array('discapacidad , discapacidad_fisica as "Presenta discap. física", discapacidad_intelectual as "Presenta discap. intelectual", discapacidad_sensorial as "Presenta discap. sensorial", presenta_discapacidad_mental as "Presenta discap. mental",carnet_conadis as "Carnet CONADIS",grado_dependencia_pam as "Grado dependencia PAM" ,motivo_dif_desplazamiento as "Dificultad Desplazamiento",enfermedad_ingreso_1 as "Enfermedad Ingreso 1", (SELECT nombre FROM pam_tipo_patologia WHERE id =pam_datos_saludnutric.tipo_patologia) as "Tipo de Patología"','nivel_hemoglobina as "Nivel de Hemoglobina", peso as "Peso (Kg.)",talla as "Talla (m)",estado_nutricional as "Estado Nutricional(IMC)"'),'pam_salud_mental'=>'trastorno_disociales as "Tras.comporta. y/o disociales", tipo_trastorno as "Tipo de trastorno"');
      break;
      case '3':
      break;
      default:
            ...
            foreach ($grupos as $key => $grupo)
            {
              if (!in_array($grupo["RESIDENTE_ID"],$residente_repite)) {
                if ($key==0) {
                  ...
                  {
                    if ($key != "RESIDENTE_ID") {
                      $grupo_html .="<tr style='border:1px solid;'><th>$key</th>";
                        $grupo_values = array_values($grupo);
                        //$grupo_html .= "<tr>";
