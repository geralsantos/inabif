...
if (isset($_GET["opcionejecutar"]) && $_GET["opcionejecutar"]!="") {
    ...
    print_r($x->executeQuery('select Ape_Paterno as "Apellido paterno",Ape_Materno as "Apellido materno", Nom_Usuario as "Nombre Usuario", (SELECT nombre FROM paises WHERE id=CarIdentificacionUsuario.Pais_Procencia) as "PaÃ­s de procedencia",(SELECT NOMDEPT FROM ubigeo WHERE coddist=CarIdentificacionUsuario.Distrito_Procedencia) as "Departamento de nac",(SELECT NOMPROV FROM ubigeo WHERE coddist=CarIdentificacionUsuario.Distrito_Procedencia) as "Provincia de nac del" ,(SELECT NOMPROV FROM ubigeo WHERE coddist=CarIdentificacionUsuario.Distrito_Procedencia) as "Distrito de nac",(CASE Sexo WHEN "h" THEN "Hombre" ELSE "Mujer" END) as Sexos,Fecha_Nacimiento,(SELECT nombre from pam_lengua_materna WHERE id = CarIdentificacionUsuario.Lengua_Materna) as "Lengua Materna" from CarIdentificacionUsuario where periodo_mes=01 and periodo_anio=2019 and residente_id= 97 and centro_id=47 order by id desc'));
 /*
